...
- Location: tests\Hooks1.spec.js:3:5
...
Expected: 9
Received: {"_apiName": "Locator", "_frame": {"_guid": "frame@9b1af7219b247ffeaef2768c16cfd28a", "_type": "Frame"}, "_selector": ".hrefch", Symbol(nodejs.util.inspect.custom): [Function anonymous]}
```
...
```ts
  ...
  2  | 
  3  | test("HomePage test", async ({page})=>{
  ...
  10 |     //HomePage
  11 |     const products = await page.locator('.hrefch')
> 12 |     expect(products).toBe(9)
     |                      ^ Error: expect(received).toBe(expected) // Object.is equality
  13 |     //Logout
