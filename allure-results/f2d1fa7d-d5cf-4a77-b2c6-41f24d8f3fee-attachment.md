...
- Location: tests\Screenshot.spec.js:16:5
...
Call log:
  - waiting for locator('//img[@title="MacBook"]')
...
```ts
  ...
  7  | 
  8  | 
  9  | // test.only('Full page screenshot', async ({page})=>{
  10 | //     await page.goto("https://www.demoblaze.com/")
  11 | //     await page.screenshot({path:'tests/screenshots/'+Date.now()+'FullPage.png',fullPage: true})
  12 |      
  13 | // })
  14 | 
  15 | 
  16 | test('Element screenshot', async ({page})=>{
  17 |      await page.goto("https://www.demoblaze.com/")
> 18 |     await page.locator('//img[@title="MacBook"]').screenshot({path:'tests/screenshots/'+Date.now()+'Image.png'})
     |                                                   ^ Error: locator.screenshot: Test timeout of 30000ms exceeded.
  19 |     
  20 | })
```